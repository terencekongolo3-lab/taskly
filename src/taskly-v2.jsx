...
// Premium (Plus) klussen — mysterieus voor non-plus
const PLUS_JOBS = [
...
// Simulated matches (deals reached)
const INIT_MATCHES = [
...
export default function Taskly() {
  const [role, setRole]   = useState(null); // null = not chosen yet, "klant" | "vakman"
  ...
  const [propDate, setPropDate] = useState(null); // { matchId, date, time }
          ...
          <div key={j.id} style={{ background:W, borderRadius:16, marginBottom:10, overflow:"hidden",
            ...
            <div style={{ position:"relative" }}>
              <div style={{ background:`linear-gradient(135deg,#1B1F3B,#312E81)`, padding:"20px 16px",
                filter:hasPlusAcc?"none":"blur(0px)" }}>
          ...
          {match.msgs.map((msg, i) => {
            ...
            return (
              <div key={i} style={{ display:"flex", justifyContent:isMe?"flex-end":"flex-start", marginBottom:10 }}>
                {!isMe && <Av label={match.proAvatar} color={match.proColor} size={28} style={{marginRight:8, flexShrink:0}}/>}
    ...
    <div style={{ overflowY:"auto", flex:1 }}>
      <div style={{ background:`linear-gradient(135deg,${N},#2d2850)`, padding:"32px 20px 28px" }}>
        <div style={{ width:72, height:72, borderRadius:20, background:O,
          display:"flex", alignItems:"center", justifyContent:"center",
          color:W, fontWeight:800, fontSize:26, marginBottom:14 }}>JD</div>
        <div style={{ fontFamily:"Georgia,serif", fontSize:22, fontWeight:700, color:W }}>Jan de Vries</div>
        <div style={{ fontSize:13, color:"rgba(255,255,255,0.55)", marginTop:4 }}>📍 Pijnacker · Lid sinds 2025</div>
